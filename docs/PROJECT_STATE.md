...
| 1.5 | **Auth polish** — email verification, password reset, OAuth providers, sessions UI, tests | ⏳ not started | ~30 files |
...
> **Convention:** when this file is updated, add a row to the update log and bump the `Last updated` date at the top. Keep the TL;DR fresh — that's what tomorrow-you reads first.
